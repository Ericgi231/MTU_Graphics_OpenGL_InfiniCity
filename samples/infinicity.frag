...
void main() 
{
	...
	float depth = pow(gl_FragCoord.z, 40);

	if (color.r > 0.99 && color.g > 0.99 && color.b > 0.99) {
		fragColor = (0.2*depth) + (1-depth) * texture(tex, out_TexCoord);
	} else {
		fragColor = (0.2*depth) + (1-depth) * diffuse * vec4(color.r, color.g, color.b, 1);
	}
	if (depth > 0.62) {
